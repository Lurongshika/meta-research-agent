...
description: Plan, execute when tools permit, validate, and interpret network meta-analyses comparing multiple interventions.
---
...
Use only execution tools available in the current Codex session. Treat upstream MCP and R tool names as workflow concepts, not guaranteed callable tools. Do not report rankings, league tables, inconsistency tests, or effect estimates unless the underlying analysis was executed and verified. When execution is unavailable, deliver a clearly labeled protocol or unexecuted script.
