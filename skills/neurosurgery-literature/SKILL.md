...
description: Design reproducible PubMed and bibliographic search strategies for neurosurgery systematic reviews using PICO and MeSH.
---
...
Use an available literature or web-search capability and cite the records actually inspected. The upstream `pubmed-mcp` name is not a guaranteed callable tool. Never describe a search as exhaustive or reproducible without recording database, complete query, filters, and search date. Verify current MeSH terms and reporting guidance against authoritative sources when needed.
