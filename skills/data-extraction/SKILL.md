...
description: Extract structured data from medical research papers for systematic reviews, including study characteristics, populations, interventions, outcomes, and validation.
---
...
Use only sources and tools actually available in the current Codex session. Do not claim a PDF was read or data was extracted unless the source was opened and inspected. Treat the named upstream helper tools as workflow concepts, not guaranteed callable tools; use equivalent available document or spreadsheet capabilities when appropriate. Preserve page-level provenance and mark uncertain values for human review.
