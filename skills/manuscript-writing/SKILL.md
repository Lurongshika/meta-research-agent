...
description: Draft and review PRISMA-compliant systematic-review and meta-analysis manuscripts for medical journals.
---
...
Use the current Codex document capabilities when the user requests an artifact. Never invent searches, registrations, study counts, effect estimates, or certainty judgments. Keep placeholders explicit until the user supplies evidence or an available tool verifies it. Current reporting guidelines should be checked against authoritative sources when accuracy may have changed.
