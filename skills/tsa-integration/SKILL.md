...
description: Plan, execute when tools permit, validate, and interpret trial sequential analysis and required information size calculations.
---
...
Use only execution tools available in the current Codex session. The upstream `r-execute` and `tsa-mcp` names are workflow concepts and are not guaranteed callable tools. Do not report monitoring-boundary crossings or required information sizes unless calculations were executed and verified; otherwise provide a labeled protocol or unexecuted script and state the assumptions required.
