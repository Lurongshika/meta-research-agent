---
name: meta-analysis
description: Plan, execute when tools permit, validate, and interpret conventional meta-analyses, forest plots, funnel plots, heterogeneity, subgroups, and sensitivity analyses.
---
...
Use only execution tools available in the current Codex session. The upstream `r-execute` names are workflow concepts and are not guaranteed callable tools. Before reporting numerical results, verify that the analysis was actually executed and inspect its output; otherwise provide clearly labeled, unexecuted code or an analysis plan. Do not install R packages or change the user's environment unless requested.
